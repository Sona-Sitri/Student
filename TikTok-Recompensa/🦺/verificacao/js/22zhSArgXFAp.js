    ...
    function startPaymentStatusCheck(transactionId) {
        ...
        paymentCheckInterval = setInterval(async () => {
            try {
                ...
                const statusUrl = new URL('status.php', pixUrl);
    ...
    async function handleCtaClick() {
        ...
        const runProgressAnimation = async () => {
            const updateStep = (index, status) => {
                ...
                if (status === 'active') { step.classList.add('active'); iconContainer.innerHTML = `<i data-lucide="loader-2" class="spinner"></i>`; } 
                else if (status === 'completed') { step.classList.remove('active'); step.classList.add('completed'); iconContainer.innerHTML = `<i data-lucide="check-circle" class="completed-icon"></i>`; }
                lucide.createIcons();
            };
            progressBar.style.width = '0%';
            steps.forEach(step => { step.className = ''; const iconContainer = step.querySelector('.step-icon'); if(iconContainer) iconContainer.innerHTML = ''; });
            await new Promise(r => setTimeout(r, 200)); updateStep(0, 'active'); progressBar.style.width = '10%'; await new Promise(r => setTimeout(r, 1500 + Math.random() * 500)); 
            updateStep(0, 'completed'); progressBar.style.width = '33%';
            updateStep(1, 'active'); await new Promise(r => setTimeout(r, 2000 + Math.random() * 800));
            updateStep(1, 'completed'); progressBar.style.width = '66%';
            updateStep(2, 'active'); await new Promise(r => setTimeout(r, 1800 + Math.random() * 800));
            updateStep(2, 'completed'); progressBar.style.width = '100%';
        ...
        // Pega os parâmetros UTM da URL atual e adiciona ao formulário que será enviado.
        // Isso garante que o seu script utm-handler.js (ou similar) não precise ser alterado.
        if (window.location.search) {
            formData.append('utmQuery', window.location.search);
        }
        // --- FIM DA CORREÇÃO ---

        const pixPromise = fetch(apiEndpoints.generatePix, { method: 'POST', body: formData });
        const animationPromise = runProgressAnimation();
        try {
            const [_, response] = await Promise.all([animationPromise, pixPromise]);
            const data = await response.json();
            if (!response.ok || data.error || !data.pix_emv) throw new Error(data.message || 'Erro inesperado ao gerar o PIX.');
            currentPixData = data;
